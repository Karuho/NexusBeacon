...
public final class ClassicGuiMutationService {
 ...
 public Result toggleEffect(ClassicBeaconLocation location,String id,boolean executorAvailable){ClassicBeaconRecord r=state.find(location);if(r==null)return Result.MISSING_BEACON;if(!executorAvailable)return Result.EXECUTOR_UNAVAILABLE;if(!r.getEffects().containsKey(id))return Result.UNSUPPORTED;return state.update(r.withEffectActive(id,!r.getActiveEffects().contains(id)))?Result.COMMITTED:Result.MISSING_BEACON;}
 public Result activateEffect(ClassicBeaconLocation location,String id){return toggleEffect(location,id,false);}
}
