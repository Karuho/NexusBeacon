...
public final class ClassicBeaconRecord {
    ...
    public ClassicBeaconRecord withEffectLevel(String id,int value,boolean activate){Map<String,Integer> next=new TreeMap<String,Integer>(effects);next.put(id,value);Set<String> enabled=new TreeSet<String>(active);if(activate)enabled.add(id);return new ClassicBeaconRecord(location,uniqueId,owner,range,level,next,enabled,trusted,protectBase,beamStyle,rangeParticles,rangeParticleType);}
    public ClassicBeaconRecord withEffectActive(String id,boolean enabled){Set<String> next=new TreeSet<String>(active);if(enabled)next.add(id);else next.remove(id);return new ClassicBeaconRecord(location,uniqueId,owner,range,level,effects,next,trusted,protectBase,beamStyle,rangeParticles,rangeParticleType);}
