...
public final class ModernFoliaSchedulerService implements SchedulerService {
    private final NexusBeaconPlugin plugin;

    public ModernFoliaSchedulerService(NexusBeaconPlugin plugin) { this.plugin = plugin; }
    ...
    public ScheduledTaskHandle runAsyncTimer(Runnable runnable, long delayTicks, long intervalTicks) {
        ScheduledTask task = Bukkit.getAsyncScheduler().runAtFixedRate(
                plugin, scheduledTask -> runnable.run(), delayTicks * 50L,
                intervalTicks * 50L, TimeUnit.MILLISECONDS);
        return new FoliaScheduledTaskHandle(task);
    }

    private static final class FoliaScheduledTaskHandle implements ScheduledTaskHandle {
        private final ScheduledTask task;
        private FoliaScheduledTaskHandle(ScheduledTask task) { this.task = task; }
