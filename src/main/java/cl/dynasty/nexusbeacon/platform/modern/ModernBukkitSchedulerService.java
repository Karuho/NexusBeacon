...
public final class ModernBukkitSchedulerService implements SchedulerService {
    private final NexusBeaconPlugin plugin;

    public ModernBukkitSchedulerService(NexusBeaconPlugin plugin) { this.plugin = plugin; }

    @Override
    public void runSync(Runnable runnable) {
        if (!plugin.isEnabled() || runnable == null) return;
    ...
    public ScheduledTaskHandle runAsyncTimer(Runnable runnable, long delayTicks, long intervalTicks) {
        BukkitTask task = plugin.getServer().getScheduler()
                .runTaskTimerAsynchronously(plugin, runnable, delayTicks, intervalTicks);
        return new BukkitScheduledTaskHandle(task);
    }

    private static final class BukkitScheduledTaskHandle implements ScheduledTaskHandle {
        private final BukkitTask task;
        private BukkitScheduledTaskHandle(BukkitTask task) { this.task = task; }
